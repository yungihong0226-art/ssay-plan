...
-- 이미 예전 버전(스키마)으로 만들어져 있다면 대신 migrate_allow_multiple_rows.sql 을 실행하세요.)
--
-- 로그인 기능이 없는 앱이라 사용자는 브라우저에 저장된 랜덤 device_id로 구분합니다.
-- 세 지표(하루 평균 공부시간 / 공부 진행률 / 시험까지 남은 기간 평균)를 여러 표로
-- 나누지 않고 한 표(study_stats)의 컬럼으로 합쳐서 저장합니다.
--
